...
def create_tr_in_winapraoch(source_ip, destination_ip, description="Routing check request"):
    wa_username, wa_password = GET_WINAPROACH_USERNAME_AND_PASSWORD()
    winapi = api.Aproach(username=wa_username, password=wa_password)
    data = f"{description} source IP:{source_ip} destination ip:{destination_ip}"
    ...
    my_TR.save()
    return my_TR.id  # Return the TR ID for confir
