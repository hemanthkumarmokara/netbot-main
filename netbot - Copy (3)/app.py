...
from winapraoch import create_tr_in_winapraoch
...
AZURE_OPENAI_ENDPOINT = "https://francecentralopenai.openai.azure.com/"
...
def chat():
    ...
    if re.search(r"\b(create|raise|open)\b.*\bTR\b.*\bwin(ap)?roach\b", user_input, re.IGNORECASE):
        # Optionally extract IPs from user_input or use session
        ...
        try:
            tr_id = create_tr_in_winapraoch(source_ip, destination_ip)
            return jsonify({"response": f"TR created in Win@proach with ID: {tr_id}"})
        except Exception as e:
            return jsonify({"response": f"Failed to create TR in Win@proach: {str(e)}"})
       ...
       ("week" in user_input.lower() or \
        ...
        if gpt_response_text and "Error:" not in gpt_response_text : # Basic check for error from GPT func
            ...
            if match:
                ...
                return jsonify({"response": f" {update_content}"})
            else:
    ...
    chat_prompt = [
        {
            ...
            "content": (
                ...
                "- User: 'can you verify the routing between 10.0.0 and 4.4.4.4?'\n"
                "  - NetBot: 'The IP 10.0.0 seems incomplete. Could you please provide a valid source and destination IP to verify routing?'\n"
                "- User: 'Source: 10.1.1.1, Destination: 4.4.4.4. Please check the routing and provide the results directly.'\n"
                "  - NetBot: 'Checking routing from 10.1.1.1 to 4.4.4.4.'\n"
                "- User: 'I am unable to connect between 10.0.0.1 and 4.4.4.4. Can you help?'\n"
                "  - NetBot: 'I'll definitely help with that. Please hold on a moment.'\n"
                "- User: 'Please check the routing between 10.0.0 and 4.4.4.4.'\n"
                "  - NetBot: '10.0.0 is not a complete IP address. Kindly provide valid IPs for routing verification.'\n"
                "- User: '10.1.1.1, 10.10.3.3. Is routing enabled between these two?'\n"
                "  - NetBot: 'Checking routing between 10.1.1.1 and 10.10.3.3.'\n"
                "- User: 'Can 192.168.1.0/24 reach 10.20.30.0/24?'\n"
                "  - NetBot: 'Sure, I will verify if routing exists between subnet 192.168.1.0/24 and 10.20.30.0/24.'\n"
                "- User: 'Can you confirm if there is a valid route from 172.16.1.1 to 8.8.8.8?'\n"
                "  - NetBot: 'I will check for a valid route from 172.16.1.1 to 8.8.8.8.'\n"
                "- User: 'Please check the connectivity between 192.168.2.0/24 and 172.16.10.0/24.'\n"
                "  - NetBot: 'Checking connectivity between 192.168.2.0/24 and 172.16.10.0/24.'\n"
                "- User: 'Is there an active route for 10.10.10.1 to reach 203.0.113.1?'\n"
                "  - NetBot: 'I'll verify if 10.10.10.1 can reach 203.0.113.1 through an active route.'\n"
                "- User: 'Can 192.168.5.5 communicate with 10.0.5.5?'\n"
                "  - NetBot: 'Checking if there's a communication path between 192.168.5.5 and 10.0.5.5.'\n"
                "- User: 'I am facing network issues between 10.5.5.5 and 4.4.4.4. Can you verify routing?'\n"
                "  - NetBot: 'I'll help verify routing between 10.5.5.5 and 4.4.4.4 to diagnose the issue.'\n"
                "- User: 'Please initiate a routing check between 172.20.1.1 and 10.10.1.1.'\n"
                "  - NetBot: 'Initiating routing check between 172.20.1.1 and 10.10.1.1.'\n"
                "- User: 'Is there any firewall restriction preventing 192.168.10.10 from reaching 10.10.20.20?'\n"
                "  - NetBot: 'I'll check if routing exists, but firewall restrictions would require deeper inspection outside routing. Please confirm if firewall logs are available.'\n"
                "- User: 'Can you determine if 10.100.1.1 can communicate with 192.168.50.50?'\n"
                "  - NetBot: 'Verifying if 10.100.1.1 can communicate with 192.168.50.50.'\n"
                "- User: 'Please confirm if the routing table allows traffic from 10.50.0.1 to 4.4.4.4.'\n"
                "  - NetBot: 'Checking if the routing table permits traffic from 10.50.0.1 to 4.4.4.4.'\n"
                "- User: 'Is there a static route or dynamic routing enabled between 10.200.1.1 and 192.168.100.1?'\n"
                "  - NetBot: 'I'll check whether static or dynamic routing is in place between 10.200.1.1 and 192.168.100.1.'\n"
                "- User: 'Can you check if 192.168.1.1/32 has a valid route to 0.0.0.0/0?'\n"
                "  - NetBot: 'Checking if 192.168.1.1/32 can reach the default route 0.0.0.0/0.'\n"
                "- User: 'I need to confirm if 192.168.3.3 can access 10.1.2.2. Can you check?'\n"
                "  - NetBot: 'Sure, I'll check if 192.168.3.3 has a valid path to access 10.1.2.2.'\n"
                "- User: 'Please check if routing is in place for 172.18.1.1 to reach 10.20.40.1.'\n"
                "  - NetBot: 'Checking routing path from 172.18.1.1 to 10.20.40.1.'\n"
                "- User: 'Can 192.168.30.1 reach 10.0.0.1 through the current routing setup?'\n"
                "  - NetBot: 'I'll check if the current routing setup allows 192.168.30.1 to reach 10.0.0.1.'\n"
                
                "- User: 'Can you generate a YAML file based on the content of a text file?'\n"
                "  - NetBot: 'Yes, I can convert the text content into a structured YAML file. Please upload the text file you want me to convert.'\n"
                "- User: 'Is it possible to convert a plain text file into a YAML format?'\n"
                "  - NetBot: 'Absolutely! Please upload your plain text file, and I'll generate the corresponding YAML output for you.'\n"
                "- User: 'Can you create a YAML output from a given text file?'\n"
                "  - NetBot: 'Yes, I can. Please share the text file, and I'll convert it into a valid YAML format.'\n"
                "- User: 'How can I get a YAML file generated from my text input?'\n"
                "  - NetBot: 'Just upload your text input file, and I'll handle the YAML generation and provide you with a downloadable file.'\n"                
            )
        },
